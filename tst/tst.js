...
// Ensure the canvas dimensions are set explicitly
canvas.width = 800;
canvas.height = 400;
...
function drawCueStick() {
  if (Math.hypot(cueBall.vx, cueBall.vy) < 0.1) {
    ...
    ctx.beginPath();
    ctx.moveTo(x1, y1);
    ctx.lineTo(x2, x2); // Fix incorrect coordinate reference
    ctx.stroke();
  }
}
...
function resetBalls() {
  ...
  const triangleStartX = canvas.width / 2; // Center the triangle horizontally
  const triangleStartY = canvas.height / 2; // Center the triangle vertically
  let ballIndex = 1; // Start numbering balls from 1

  for (let row = 0; row < 4; row++) {
    for (let col = 0; col <= row; col++) {
      const xOffset = col * 2 * BALL_RADIUS - row * BALL_RADIUS;
      const yOffset = row * 2 * BALL_RADIUS;

      balls.push({
        x: triangleStartX + xOffset,
        y: triangleStartY + yOffset,
        vx: 0,
        vy: 0,
        number: ballIndex, // Assign a number to each ball
        color: `hsl(${ballIndex * 36}, 70%, 50%)`
      });
      ballIndex++;
      if (ballIndex > 10) break; // Limit to 10 balls
    }
...
// Add event listener for clicking on balls
canvas.addEventListener('click', (e) => {
  ...
  balls.forEach((ball) => {
    ...
    if (dist < BALL_RADIUS) {
      alert(`You clicked on ball number ${ball.number}`); // Display the ball number
    }
  });
  ...
  if (cueDist < BALL_RADIUS) {
    alert('You clicked on the cue ball'); // Display a message for the cue ball
  }
});
